...
export const MediaCast: React.FC<MediaCastProps> = ({ mediaId }) => {
  ...
  React.useEffect(() => {
    ...
    const fetchCast = async () => {
      setLoading(true);
      try {
        const isTv = String(mediaId).startsWith('tv-');
        const response = isTv 
          ? await tvService.getTVCredits(mediaId)
          : await movieService.getMovieCredits(mediaId);

        if (active) {
          setCast(response.cast ? response.cast.slice(0, 18) : []);
        }
      } catch (e) {
        console.warn('Failed to load media cast:', e);
        if (active) setCast([]);
      } finally {
